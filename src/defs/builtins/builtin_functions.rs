...
type BuiltinValueBased = Arc<
    dyn Fn(Vec<MascalValue>, Rc<RefCell<ExecutionData>>) -> Result<Option<MascalValue>, MascalError>
        + Send
        + Sync,
>;
type BuiltinExpressionBased = Arc<
    dyn Fn(
...
pub enum BuiltinFunction {
    ValueBased {
        ...
        execution: BuiltinValueBased,
    },

    ExpressionBased {
        fixed_argument_types: Vec<Vec<MascalTypeKind>>,
        supports_dynamic_arguments: bool,
        execution: BuiltinExpressionBased,
    },
}
...
pub static BUILT_IN_FUNCTION_TABLE: Lazy<FxHashMap<String, Arc<BuiltinFunction>>> = Lazy::new(
    || {
        ...
        define_builtin_function!(
            BuiltinFunction::new_value_based,
            "Write",
            map,
            vec![],
            true,
            |args, _| {
                ...
                println!("{}", args.last().unwrap().as_string()?);
                Ok(None)
            }
        );
            ...
            |args, _| {
                match args.first().unwrap() {
                    MascalValue::StaticArray(v) => {
                        ...
                        Ok(Some(MascalValue::StaticArray(
                    ...
                    MascalValue::DynamicArray(v) => {
                        let mut idx: Vec<usize> = (0..v.len()).collect();
                        idx.shuffle(&mut rand::rng());
                        Ok(Some(MascalValue::DynamicArray(
                            idx.into_iter().map(|i| v[i].clone()).collect(),
                        )))
                    }
        ...
        define_builtin_function!(
            ...
            |args, _| {
                match args.first().unwrap() {
                    MascalValue::StaticArray(v) => {
                        let idx: &Rc<RefCell<Option<MascalValue>>> =
                            &v[rand::rng().random_range(0..=v.len())];
            ...
            |args, _| {
                match (args.first().unwrap(), args.last().unwrap()) {
                    ...
                    (MascalValue::Integer(i), MascalValue::Float(f)) => {
                        ...
                        Ok(Some(MascalValue::Float(rand::random_range(
                            i.as_f64()..=(*f),
                        ))))
                    }
                    ...
                    (MascalValue::Integer(i1), MascalValue::Integer(i2)) => {
                        ...
                        Ok(Some(MascalValue::Integer(IntegerNum::new(
                            rand::random_range(i1.to_i128()..=i2.to_i128()),
                        ))))
                    }
        ...
        define_builtin_function!(
            ...
            "Type_cast",
            map,
            vec![vec![MascalTypeKind::Type], vec![MascalTypeKind::Dynamic]],
            true,
            |args, _| {
                let MascalValue::Type(extract_type) = args[0].clone() else {
                    unreachable!()
                };
        ...
        define_builtin_function!(
            ...
            true,
            |args, _| { Ok(Some(MascalValue::Type(args[0].as_atomic_mascal_type()?))) }
        );
        ...
        define_builtin_function!(
            ...
            "Ln",
            map,
            vec![vec![MascalTypeKind::Float, MascalTypeKind::Integer]],
            false,
            |args, _| {
                match args.first().unwrap() {
        ...
        define_builtin_function!(
            ...
            |args, _| {
                let (min, max) = match (&args[1], &args[2]) {
                    ...
                    (MascalValue::Float(f), MascalValue::Integer(i)) => (*f, i.as_f64()),
                    (MascalValue::Integer(i), MascalValue::Float(f)) => (i.as_f64(), *f),
        ...
        define_builtin_function!(
            ...
            false,
            |args, _| {
                let MascalValue::String(main_str) = &args[0] else {
                    unreachable!()
                };
                let MascalValue::String(target_str) = &args[1] else {
                    unreachable!()
                };
                let MascalValue::String(sub_str) = &args[2] else {
                    unreachable!()
                };
                Ok(Some(MascalValue::String(Arc::from(
                    main_str.replace(&*target_str.clone(), &sub_str.clone()),
                ))))
            }
        );
        ...
        define_builtin_function!(
            ...
            |args, _| {
                ...
                let MascalValue::Integer(min_index) = &args[1] else {
                    unreachable!()
                };
                let MascalValue::Integer(max_index) = &args[2] else {
                    unreachable!()
                };
                ...
                if min < 0 {
                    min += main_str.len() as i128;
                }
                if max < 0 {
                    max += main_str.len() as i128;
                }
                if (min < 0 || min >= main_str_len) || (max < 0 || max >= main_str_len) || min > max
                {
                    return Err(MascalError {
                        ...
                        character: 0,
                        source: format!(
                            "Cannot take a substring in the range of from {} to {}",
                            min, max
                        ),
                    });
                }
                let slice: &str = &main_str[min as usize..=max as usize];
                Ok(Some(MascalValue::String(Arc::from(slice))))
            }
        );
        ...
        define_builtin_function!(
            ...
            false,
            |args, _| {
                let MascalValue::String(sep) = &args[1] else {
                    unreachable!()
                };
                match &args[0] {
                    MascalValue::StaticArray(v) => {
        ...
        define_builtin_function!(
            ...
            |args, _| {
                match args.first().unwrap() {
                    MascalValue::Integer(i) => Ok(Some(MascalValue::Integer(IntegerNum::new(
                        i.as_f64().exp().round() as i128,
                    )))),
        ...
        define_builtin_function!(
            ...
            |args, _| {
                ...
                for arg in args {
                    encountered_float = encountered_float
                        || sum_internal(
        ...
        define_builtin_function!(
            ...
            "Contains",
            map,
            vec![vec![MascalTypeKind::String], vec![MascalTypeKind::String]],
            false,
            |args, _| {
                let MascalValue::String(string) = &args[0] else {
                    unreachable!()
                };
                let MascalValue::String(composite) = &args[1] else {
                    unreachable!()
                };
                        ...
                        for (index, value) in values.iter().enumerate() {
                            if let Some(unwrapped_value) = &*value.borrow() {
                                if unwrapped_value.is_equal(&args[1]) {
                                    return Ok(Some(MascalValue::Integer(IntegerNum::new(
                                        index as i128,
                                    ))));
                                }
                            }
        ...
        define_builtin_function!(
            ...
            |args, _| {
                match &args[0] {
                    ...
                    MascalValue::String(s) => Ok(Some(MascalValue::Integer(IntegerNum::new(
                        s.matches(&args[1].as_string()?).count() as i128,
                    )))),
        ...
        define_builtin_function!(
            ...
            false,
            |args, _| {
                let MascalValue::String(inner_str) = &args[0] else {
                    unreachable!()
                };
                Ok(Some(MascalValue::String(Arc::from(
                    inner_str.to_uppercase(),
                ))))
            }
        );
        ...
        define_builtin_function!(
            ...
            false,
            |args, _exec_data| { get_dimensions(&Some(args.first().unwrap().clone()), 0) }
        );
        ...
        define_builtin_function!(
            ...
            false,
            |args, _exec_data| { get_sizes(&Some(args.first().unwrap().clone()), Vec::new()) }
        );
            ...
            |args, _exec_data| {
                ...
                if args.len() == 1 {
                    return Err(MascalError {
                        ...
                        character: 0,
                        source: String::from(
                            "Expected a value (element) to be pushed to the array",
                        ),
                    });
                }
            ...
            |args, _exec_data| {
                ...
                if args.len() == 1 {
                    return Err(MascalError {
                        ...
                        character: 0,
                        source: String::from(
                            "Expected an array of elements to be appended to the array",
                        ),
                    });
                }
                if let MascalValue::DynamicArray(mut v) = array {
                        ...
                        counter += match arg {
                            ...
                            _ => {
                                return Err(MascalError {
                                    ...
                                    character: 0,
                                    source: String::from(
                                        "Expected an array to push but got an atomic type",
                                    ),
                                });
                            }
                        };
        ...
        define_builtin_function!(
            BuiltinFunction::new_expresion_based,
            "Swap",
            map,
            vec![],
            true,
            |args, exec_data| {
                if args.len() != 2 {
                    return Err(MascalError {
                        ...
                        character: 0,
                        source: format!(
                            "Expected 2 variable names but got {} arguments instead",
                            args.len()
                        ),
                    });
                }
                let varname1: &String = match args[0] {
                    MascalExpression::Symbolic(s) => s,
                    _ => {
                        return Err(MascalError {
                            error_type: MascalErrorType::ArgumentError,
                            line: 0,
                            character: 0,
                            source: String::from(
                                "Expected a variable name as first argument but got something else instead",
                            ),
                        });
                    }
                };
                let varname2: &String = match args[1] {
                    ...
                    _ => {
                        return Err(MascalError {
                            ...
                            character: 0,
                            source: String::from(
                                "Expected a variable name as second argument but got something else instead",
                            ),
                        });
                    }
                };
                if let Some(wrapped_vartable) = &exec_data.borrow().variable_table {
                    ...
                    return Err(MascalError {
                        ...
                        line: 0,
                        source: String::from(
                            "Expected variable names but got at least one unknown",
                        ),
                    });
                }
                unreachable!()
            ...
            |args, exec_data| {
                for arg in args {
                    let varname: &str = match arg {
                        MascalExpression::Symbolic(s) => s.as_str(),
                        ...
                        _ => {
                            return Err(MascalError {
                                ...
                                character: 0,
                                source: String::from(
                                    "Expected an identifier for a variable name but found other expression",
                                ),
                            });
                        }
                    };
                    ...
                    if let Some(unwrapped_vardata) = mutable_borrow_vartable.get_mut(varname) {
                        if !unwrapped_vardata.array_dimensions.is_empty() {
                            return Err(MascalError {
                                ...
                                character: 0,
                                source: format!(
                                    "The variable called {:?} is an array type which is unsupported",
                                    varname
                                ),
                            });
                        }
                        let atomic_type: &MascalType =
                            unwrapped_vardata.atomic_variable_type.as_ref();
                        ...
                        let read_value: MascalValue = match atomic_type {
                            MascalType::Integer => {
                                ...
                                if int.is_err() {
                                    return Err(MascalError {
                                        ...
                                        character: 0,
                                        source: String::from(
                                            "The user input cannot be parsed as an integer",
                                        ),
                                    });
                                }
                            ...
                            MascalType::Float => {
                                ...
                                if int.is_err() {
                                    return Err(MascalError {
                                        ...
                                        character: 0,
                                        source: String::from(
                                            "The user input cannot be parsed as a float",
                                        ),
                                    });
                                }
                                MascalValue::Float(int.unwrap())
                            }
                            MascalType::Boolean => match input_str {
                                ...
                                _ => {
                                    return Err(MascalError {
                                        ...
                                        character: 0,
                                        source: String::from(
                                            "The user input cannot be parsed as a boolean",
                                        ),
                                    });
                                }
                            },
                            ...
                            _ => {
                                return Err(MascalError {
                                    ...
                                    character: 0,
                                    source: String::from(
                                        "This type of variable is unsupported when reading a user input",
                                    ),
                                });
                            }
                        };
