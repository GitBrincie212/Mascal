...
macro_rules! define_builtin_function {
    (
        ...
        let func: BuiltinFunction =
            $builtin_construct($supports_dynamic_arguments, $fixed_args, $func);
        $map.insert(String::from($name).to_lowercase(), func);
    };
}

pub static BUILT_IN_FUNCTION_TABLE: Lazy<FxHashMap<String, BuiltinFunction>> = Lazy::new(|| {
    let mut map: FxHashMap<String, BuiltinFunction> = FxHashMap::default();
    ...
    define_builtin_function!(
        ...
        |args, _| {
            let (min, max) = match (&args[1], &args[2]) {
                (MascalValue::Integer(i1), MascalValue::Integer(i2)) => (i1.as_f64(), i2.as_f64()),
