...
fn merge_paths(input_path: &Path, lang_dir: &Path) -> Option<PathBuf> {
    ...
    if absolute_path.is_err() {
        println!("\x1b[1;31mUnexpected error when trying to merge the relative path of the provided file \x1b[0m");
        return None;
    }
...
fn main() {
    ...
    if args.len() <= 1 {
        println!("\x1b[1;31mPlease provide at least one argument (that being the file to run)\x1b[0m");
        exit(64)
    }
    if lang_path.is_err() {
    ...
    if extracted_lang_path.parent().is_none() {
        println!("\x1b[1;31mUnexpected error when trying to get the executable file's directory \x1b[0m");
        return;
    }
    ...
    if extension.is_none() {
        println!("\x1b[1;31mUnexpected Error when determing the extension of the provided file\x1b[0m");
        return;
    }
