...
fn get_parameters_of_func(
    token_sequence: &TokenSequence, mut curr_index: usize
    ...
    while !token_sequence.is_of(TokenType::CloseParen, curr_index) {
        ...
        let next_token_type = &token_sequence.get_token(curr_index + 1)
        ...
        if curr_token.token_type == TokenType::Identifier {
            ...
            if parameter_name.is_some() {
                return Err(MascalError {
                    ...
                    source: String::from("the keyword MUT should be followed before the variable's name")
                })
            } else if is_mutable {
                return Err(MascalError {
                    error_type: MascalErrorType::ParserError,
