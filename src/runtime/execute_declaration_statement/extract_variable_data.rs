...
pub fn extract_variable_data(
    base: MascalExpression, variable_table: Rc<RefCell<VariableTable>>,
    ...
    let varname: String = match base {
        MascalExpression::SymbolicExpression(name) => name,
        other => {
    ...
    let vardata = vartable_borrow.get(&varname)
        .ok_or_else(|| MascalError {
            error_type: MascalErrorType::RuntimeError,
            line: 0,
            character: 0,
            source: format!("Expected a defined variable but got an unknown called {}", varname)
        })?
        .clone();
    ...
    if vardata.value.is_none() && layers.len() >= 1 {
        return Err(MascalError {
            error_type: MascalErrorType::RuntimeError,
            character: 0,
            line: 0,
            source: format!("Cannot assign an array to a uninitialized variable {}", varname)
        });
    }
