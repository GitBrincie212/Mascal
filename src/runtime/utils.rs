...
pub fn make_array(dims: &[usize], dyns: &[bool]) -> MascalValue {
    if dims.len() == 1 {
        let len = dims[0];
        let slots = (0..len)
    ...
    let slots = (0..len)
        .map(|_| Rc::new(RefCell::new(Some(make_array(tail_dims, tail_dyns)))))
        .collect::<Vec<_>>();
...
pub fn get_sizes(
    ...
    let converted_size = sizes
        .iter()
        .map(|x| {
            Rc::new(RefCell::new(Some(MascalValue::Integer(IntegerNum::new(
                *x as i128,
            )))))
        })
        .collect::<Vec<_>>()
    ...
    ($values: expr, $array_type: expr, $value_type_init: expr) => {{
        for val in $values.clone() {
            ...
            if let Some(unwrapped_val) = &*val_mut_borrow {
                ...
                if !is_dynamic_type && unwrapped_val.is_type_of(&deref_array_type) {
                    return Err(MascalError {
                        ...
                        source: format!(
                            ...
                            $array_type.as_string(),
                            unwrapped_val.as_string()?
                        ),
                    });
                }
                if is_dynamic_type {
                ...
                *val_mut_borrow = Some(execute_processed_typecast(
                    deref_array_type,
                    unwrapped_val.clone(),
                )?);
                continue;
