...
use crate::{
    as_mascal_atomic_type_array_impl, as_mascal_type_array_impl, as_string_array_impl,
    as_type_string_array_impl, atomic_type_array_impl, uninit_cell_error,
};
...
fn is_expected_array_internal(
    outer_value: Rc<RefCell<Option<MascalValue>>>,
    sizes: Rc<[usize]>,
    dynamics: Rc<[bool]>,
    curr: usize,
) -> Result<(), MascalError> {
            ...
            MascalValue::StaticArray(values) => {
                if curr < sizes.len() {
                    ...
                    return Err(MascalError {
                        ...
                        character: 0,
                        source: String::from(
                            "The current array type is deeper than initialized to be",
                        ),
                    });
                }
                for val in values.clone().into_iter() {
                    is_expected_array_internal(
                        val.clone(),
                        sizes.clone(),
                        dynamics.clone(),
                        curr + 1,
            ...
            _ => {
                if sizes.is_empty() && curr >= 1 {
                    return Err(MascalError {
                        ...
                        character: 0,
                        source: String::from(
                            "Cannot assign an array to a value that hasn't initialized an array size",
                        ),
                    });
                }
                if curr < sizes.len() {
                    return Err(MascalError {
                        ...
                        source: format!("Expected a {} but got an atomic type instead", {
                            ...
                            if is_dynamic_current {
                                ...
                                format!("static array with {} dimension(s)", current_size)
                            }
                        }),
                    });
                }
...
impl MascalValue {
    pub fn is_equal(&self, other: &MascalValue) -> bool {
        match (self, other) {
            (MascalValue::Integer(i1), MascalValue::Integer(i2)) => i1.to_i128() == i2.to_i128(),
            ...
            (MascalValue::DynamicArray(values1), MascalValue::DynamicArray(values2)) => {
                ...
                values1.iter().zip(values2.iter()).all(|(v1, v2)| {
                    ...
                    if v1_borrow.is_none() && v2_borrow.is_none() {
                        return true;
                    }
                    false
                })
    ...
    pub fn as_string_inner(&self, quote_string: bool) -> Result<String, MascalError> {
        match self {
            MascalValue::String(s) => Ok(if quote_string {
                format!("{:?}", s.deref().to_string())
            } else {
                s.deref().to_string()
            }),
            MascalValue::Integer(i) => Ok(i.as_string()),
            MascalValue::Float(f) => Ok(if f.floor() == *f {
                format!("{}.0", f)
            } else {
                f.to_string()
            }),
            MascalValue::Boolean(b) => {
                if *b {
                    Ok(String::from("TRUE"))
                } else {
            ...
            MascalValue::Type(t) => Ok(t.as_string()),
        }
    }
    ...
    pub fn is_array(&self) -> bool {
        matches!(
            self,
            MascalValue::DynamicArray(_) | MascalValue::StaticArray(_)
        )
    }
